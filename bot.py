...
async def sync(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.followup.send(f"✅ Synced {len(guild_synced)} guild + {len(global_synced)} global commands.\nWait 1-2 minutes.", ephemeral=True)
...
@bot.tree.command(name="join", description="Join game")
...
@bot.tree.command(name="start", description="Start game")
...
@bot.tree.command(name="play", description="Play card")
@app_commands.describe(card_index="0-5", stat="Stat")
